...
var wdd = {
  ...
  _search: function(dd, q) {
    ...
    var res = [], keys = dd.opts.searchKeys, len = keys.length, qlen = q.length;
  ...
  _requestList: function(dd) {
    ...
    ajax.post(dd.opts.url, extend({str: q}, dd.opts.params || {}), {onDone: function(data) {
      if (dd.opts.onDataLoaded) {
        data = dd.opts.onDataLoaded(data);
      }
  ...
  _renderList: function(dd, data, showEmpty, append) {
    ...
    for (var i = 0, l = data.length; i < l; ++i) {
      ...
      var img = item[3] ? '<b class="fl_l wddi_thumb' + onlineClass + '"><img class="wddi_img" src="' + (isArray(item[3]) ? '/images/community_' + (window.devicePixelRatio >= 2 ? 100 : 50) + '.png' : item[3]) + '" /></b>' : '', cl;
      if (shown) {
        cl = 'wddi';
      } else {
        cl = 'wddi_over';
...
<div class="' + cl + '" onmousedown="WideDropdown.over(\'' + dd.id + '\', \'' + clean(id) + '\'); WideDropdown.select(\'' + dd.id + '\', event)" onmousemove="WideDropdown.over(\'' + dd.id + '\', \'' + clean(id) + '\')" id="wddi' + id + '_' + dd.id + '" onclick="">\
  ...
  _checkScroll: function(dd) {
    if (dd.scroll === void 0) {
      ...
      stManager.add(['ui_common.css', 'ui_common.js'], function() {
        dd.scroll = new uiScroll(dd.list, {global: true});
      });
    }
  },
  ...
  _updateImgs: function(dd, noAnim) {
    ...
    if (noAnim === true) {
      ...
      animate(n ? dd.img.insertBefore(o, n) : dd.img.appendChild(o), {opacity: 1}, 150, function() {
        if (dd.imgRand === r) {
          val(dd.img, t);
        }
      });
...
<div class="wdd_add2">\
  <table cellspacing="0" cellpadding="0"><tr>\
    ...
    dd.listWrap = el.insertBefore(ce('div', {className: 'wdd_lwrap', innerHTML: '<div class="wdd_list"></div>'}, {display: 'none', width: opts.width || getSize(el)[0]}), el.firstChild);
  ...
  choose: function(id, e, item, noFire) {
    ...
    var res = dd.opts.onChange && !noFire ? dd.opts.onChange(1, sel) : true, noAnim = (res === 1);
    if (res !== 0) {
